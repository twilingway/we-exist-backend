...
export class AuthController {
    ...
    googleAuthCallback(@Req() req: Request, @Res() res: Response) {
        ...
        return res.redirect(`http://localhost:3003/api/auth/success-google?token=${token}`);
    }
    ...
    yandexAuthCallback(@Req() req: Request, @Res() res: Response) {
        const token = req.user['accessToken'];
        return res.redirect(`http://localhost:3003/api/auth/success-yandex?token=${token}`);
    }
